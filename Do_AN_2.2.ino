...
#define FIREBASE_HOST "do-an-2-2854c-default-rtdb.firebaseio.com"
#define FIREBASE_AUTH "PuzAqtheYDnyGUhB0GatFO4ZqQxdKOPna9H0trhD"
...
//char ssid[] = "Do An 2";
//char pass[] = "123456789";
String apikey = "0l8bjCsPdjCAsoLdnYrIwL2I7isfTA57";
String citycode = "1-352954_1_AL";
...
void getWeather()
{
  ...
  String url = "/forecasts/v1/daily/1day/";
  url += citycode;
  url += "?apikey=";
  url += apikey;
  ...
  if (client.connect(server, 80))
  {
    ...
    client.print(String("GET ") + url + " HTTP/1.1" + "\r\n" +
                 "Host: " + String(server) + "\r\n" +
                 "Connection: close\r\n\r\n");
    // THEO THỨ TỰ NHƯ TRONG LECTURE 5
...
void filter()
{
  ...
  line.toCharArray(txt , line.length() + 1); // CHUYỂN STRING SANG CHAR
  Serial.print(txt);
  JsonObject & root = jsonBuffer.parseObject(txt);
  Serial.print(txt);
  String weaD = root["DailyForecasts"][0]["Day"]["PrecipitationType"];
  String weaN = root["DailyForecasts"][0]["Night"]["PrecipitationType"];
  if (weaD != 0 && weaN != 0) {
    Serial.println("\n");
    Serial.println(" \t khản năng mua trong sáng nay " + weaD);
    Serial.println(" \t khản năng mua trong tối nay " + weaN);
    if (weaD == "Rain") {
...
void PhoiRut ()
{
  ...
  if (  ( (Read.AnhSang == LOW) && (Read.Mua == HIGH) && (Read.Button == HIGH)  ) && (check == 0)) {
    ...
    Firebase.setString("/ESP8266/PHOI/state", "ON");
  ...
  if ( (  (Read.AnhSang == HIGH) || (Read.Mua == LOW)  || (Read.Button == LOW) ) && (check == 1) ) {
    ...
    Firebase.setString("/ESP8266/PHOI/state", "OFF");
...
void setup()
{
  ...
  lcd.begin();
  lcd.home();
  Blynk.begin(auth, ssid, pass, IPAddress(192, 168, 0, 9  ), 8080);
  pinMode(5, INPUT);
  pinMode(4, INPUT);
